...
def convert_type(gt_result, dt_result, filter_id=None, cvt_img_id=None, cvt_box=None, split_box=None, cvt_score=None,
    ...
    if cvt_img_id is not None:
        ...
        if cvt_img_id == '.':
            ...
            dt_result['image_id'] = dt_result['image_id'].applymap(lambda x: m[x])
        else:
...
def main():
    args = parse_args()
    convert = {} if args.convert is None else args.convert
    options = {} if args.options is None else args.options
    columns = {} if args.columns is None else args.columns
