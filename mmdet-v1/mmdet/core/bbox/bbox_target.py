...
def bbox_target_single_tsd(pos_bboxes,
    ...
    if num_pos > 0:
        ...
        pc_cls_loss = F.relu(-(TSD_cls_score_soft[np.arange(len(TSD_labels)), TSD_labels] - cls_score_soft[
            np.arange(len(TSD_labels)), labels].detach() - cls_pc_margin))
        ...
        sibling_head_bboxes = delta2bbox(pos_bboxes, bbox_pred_[np.arange(num_pos), labels[:num_pos]],
                                         means=target_means, stds=target_stds)
        TSD_head_bboxes = delta2bbox(TSD_pos_rois[:, 1:], TSD_bbox_pred_[np.arange(num_pos), TSD_labels[:num_pos]],
                                     means=target_means, stds=target_stds)
