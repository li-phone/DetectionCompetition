...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='''
            fmt [xml]: 
                ann_or_dir: xml_dir, options: img_dir, bgcat(background label), info=None, license=None
            fmt [test_dir]: 
                ann_or_dir: train_ann, options: test_dir(test img dir), info=None, license=None
            fmt [json, csv]: 
                ann_or_dir: ann_file, options: img_dir, bgcat(background label), info=None, license=None
        '''
    )
    parser.add_argument(
        '--fmt',
...
def main():
    ...
    if args.fmt == 'xml':
        anns = xml2list(args.ann_or_dir, **kwargs)
        transform2coco(anns, args.save_name, **kwargs)
    elif args.fmt == 'test_dir':
        imgdir2coco(args.ann_or_dir, args.save_name, **kwargs)
    elif args.fmt == 'csv' or args.fmt == 'json':
        transform2coco(args.ann_or_dir, args.save_name, **kwargs)
