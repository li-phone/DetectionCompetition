...
class Config(object):
    ...
    # data module
    img_dir = "data/track/panda_round1_test_202104_B/"
    test_file = "data/track/panda_round1_test_B_annos_20210222/person_bbox_test_B.json"
    save_file = "work_dirs/track/test_B-best-r50-mst_slice-scale_3-2.json"
...
def process(image, **kwargs):
    ...
    for compose, slice_num in zip(config.composes, config.max_slice_num):
        results = compose({'img_prefix': config.img_dir, 'img_info': {'filename': image[0]}})
