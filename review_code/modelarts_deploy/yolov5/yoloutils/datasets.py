...
from yoloutils.utils import xyxy2xywh, xywh2xyxy
...
def test_pipeline(image, img_size=640):
    if isinstance(image, str):
        ...
        raise Exception('only support str or np.array image format')
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            self.img_files = [x.split(',')[0] for x in f]
        ...
        self.label_files = [x.split(',')[1] for x in f]
